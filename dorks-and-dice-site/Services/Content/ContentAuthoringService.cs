...
public sealed class ContentAuthoringService : IContentAuthoringService
{
    ...
    public ContentAuthoringEditViewModel GetNew(string? sourceKey)
    {
        ...
        var defaultModeId = _siteModeRegistry.All.FirstOrDefault()?.Id
            ?? throw new InvalidOperationException("At least one normal site mode must be registered before creating content.");
