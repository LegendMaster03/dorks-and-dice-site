...
public sealed class DatabaseToolRegistry : IToolRegistry
{
    ...
    public async Task<IReadOnlyList<ToolRegistration>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        ...
        await using (connection)
        {
            ...
            while (await reader.ReadAsync(cancellationToken)) tools.Add(ReadRegistration(reader, provider));
            return tools.OrderBy(tool => tool.DisplayName, StringComparer.OrdinalIgnoreCase).ToArray();
        }
    }

    public Task<ToolRegistration?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default) =>
        GetSingleAsync("tool_id = @tool_id", (command, provider) => AddIdParameter(command, "@tool_id", id, provider), cancellationToken);

    public Task<ToolRegistration?> GetBySlugAsync(string slug, CancellationToken cancellationToken = default) =>
        GetSingleAsync("lower(tool_slug) = lower(@tool_slug)", (command, _) => AddParameter(command, "@tool_slug", slug), cancellationToken);
    ...
    public async Task SaveAsync(ToolRegistration registration, CancellationToken cancellationToken = default)
    {
        ...
        if (duplicate is not null && duplicate.Id != registration.Id) throw DuplicateSlug(registration.Slug);
        ...
        await using (connection)
        {
            ...
            command.CommandText = """
                ...
                (tool_id, tool_slug, tool_display_name, tool_description, tool_integration_type,
                 tool_integration_contract_version, tool_operator_contract_version, tool_operator_manifest_path,
                 tool_upstream_base_url, tool_frontend_entry_point, tool_health_path, tool_modes,
                ...
                ON CONFLICT(tool_id) DO UPDATE SET
                    ...
                    tool_operator_contract_version = excluded.tool_operator_contract_version,
                    tool_operator_manifest_path = excluded.tool_operator_manifest_path,
            ...
            AddParameter(command, "@tool_operator_contract_version", registration.OperatorContractVersion);
            AddParameter(command, "@tool_operator_manifest_path", registration.OperatorManifestPath);
            ...
            catch (PostgresException ex) when (ex.SqlState == PostgresErrorCodes.UniqueViolation && string.Equals(ex.ConstraintName, "ux_tool_registration_slug_ci", StringComparison.Ordinal)) { throw DuplicateSlug(registration.Slug, ex); }
            catch (SqliteException ex) when (ex.SqliteExtendedErrorCode == 2067) { throw DuplicateSlug(registration.Slug, ex); }
        }
    }
    ...
    private async Task<ToolRegistration?> GetSingleAsync(string predicate, Action<DbCommand, RegistryProvider> addParameter, CancellationToken cancellationToken)
    {
        var (connection, provider) = CreateConnection();
        await using (connection)
        {
            ...
            await using var reader = await command.ExecuteReaderAsync(cancellationToken);
            return await reader.ReadAsync(cancellationToken) ? ReadRegistration(reader, provider) : null;
        }
    }
    ...
    private (DbConnection Connection, RegistryProvider Provider) CreateConnection()
    {
        ...
        return source.Provider.ToLowerInvariant() switch
        {
            ...
            _ => throw new NotSupportedException($"Tool registry content database provider '{source.Provider}' is not supported.")
        };
    }

    private static ToolRegistration ReadRegistration(DbDataReader reader, RegistryProvider provider) => new()
    {
        Id = ReadGuid(reader.GetValue(0)), Slug = reader.GetString(1), DisplayName = reader.GetString(2), Description = ReadNullableString(reader, 3),
        IntegrationType = (ToolIntegrationType)Convert.ToInt32(reader.GetValue(4), CultureInfo.InvariantCulture),
        IntegrationContractVersion = reader.IsDBNull(5) ? null : Convert.ToInt32(reader.GetValue(5), CultureInfo.InvariantCulture),
        OperatorContractVersion = reader.IsDBNull(6) ? null : Convert.ToInt32(reader.GetValue(6), CultureInfo.InvariantCulture),
        OperatorManifestPath = ReadNullableString(reader, 7), UpstreamBaseUrl = ReadNullableString(reader, 8), FrontendEntryPoint = ReadNullableString(reader, 9), HealthPath = ReadNullableString(reader, 10),
        Modes = ReadModes(reader.GetValue(11), provider), AllowAnonymous = Convert.ToBoolean(reader.GetValue(12), CultureInfo.InvariantCulture), Enabled = Convert.ToBoolean(reader.GetValue(13), CultureInfo.InvariantCulture),
        CreatedAt = ReadTimestamp(reader.GetValue(14)), UpdatedAt = ReadTimestamp(reader.GetValue(15))
    };

    private static Guid ReadGuid(object value) => value switch { Guid guid => guid, string text => Guid.Parse(text), _ => Guid.Parse(Convert.ToString(value, CultureInfo.InvariantCulture) ?? throw new InvalidOperationException("Tool registration ID is null.")) };
    ...
    private static List<string> ReadModes(object value, RegistryProvider provider)
    {
        if (provider == RegistryProvider.PostgreSql)
            return value switch { string[] values => values.ToList(), IEnumerable<string> values => values.ToList(), _ => throw new InvalidOperationException("Tool registration modes are not a PostgreSQL text array.") };
        return JsonSerializer.Deserialize<List<string>>(Convert.ToString(value, CultureInfo.InvariantCulture) ?? "[]") ?? [];
    }
    ...
    private static void AddIdParameter(DbCommand command, string name, Guid value, RegistryProvider provider) => AddParameter(command, name, provider == RegistryProvider.Sqlite ? value.ToString("D") : value);
    private static void AddModesParameter(DbCommand command, string name, IReadOnlyCollection<string> modes, RegistryProvider provider)
    {
        var parameter = command.CreateParameter(); parameter.ParameterName = name;
        if (provider == RegistryProvider.PostgreSql) { parameter.Value = modes.ToArray(); if (parameter is NpgsqlParameter npgsqlParameter) npgsqlParameter.NpgsqlDbType = NpgsqlDbType.Array | NpgsqlDbType.Text; }
        else parameter.Value = JsonSerializer.Serialize(modes);
        command.Parameters.Add(parameter);
    }
    private static void AddTimestampParameter(DbCommand command, string name, DateTimeOffset value, RegistryProvider provider) => AddParameter(command, name, provider == RegistryProvider.Sqlite ? value.ToUniversalTime().ToString("O", CultureInfo.InvariantCulture) : value.ToUniversalTime().UtcDateTime);
    ...
    private static InvalidOperationException DuplicateSlug(string slug, Exception? innerException = null) => new($"A tool with slug '{slug}' already exists.", innerException);
    private enum RegistryProvider { Sqlite, PostgreSql }
}
