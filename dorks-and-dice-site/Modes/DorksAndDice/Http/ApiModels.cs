...
public static class DorksAndDiceApiMapper
{
    ...
    {
        var roles = campaign.Memberships.Where(membership => membership.UserId == userId && membership.Status == CampaignMembershipStatus.Active)
        ...
        return new CampaignSummaryResponse(campaign.Id, campaign.Name, campaign.Status.ToString(), roles);
    }

    public static CampaignDetailsResponse ToDetails(Campaign campaign, Guid userId)
    {
        var currentRoles = campaign.Memberships.Where(membership => membership.UserId == userId && membership.Status == CampaignMembershipStatus.Active)
            .SelectMany(membership => membership.Roles).Select(role => role.Role).Distinct(StringComparer.Ordinal).OrderBy(role => role, StringComparer.Ordinal).ToArray();
    ...
    {
        var campaigns = character.CampaignAssociations.Where(association => association.Status == CampaignCharacterAssociationStatus.Active)
            .OrderBy(association => association.Campaign.Name)
            .Select(association => new CharacterCampaignResponse(association.CampaignId, association.Campaign.Name)).ToArray();
        return new CharacterSummaryResponse(character.Id, character.Name, character.Status.ToString(), character.ArchivedAt, campaigns);
    }
}
