...
[Authorize]
public sealed class PermanentDeletionController(IDorksAndDiceDeletionService deletionService) : Controller
{
    private readonly IDorksAndDiceDeletionService _deletionService = deletionService;

    [HttpPost("/characters/{characterId:guid}/delete")]
    ...
    [HttpPost("/campaigns/{campaignId:guid}/delete")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> DeleteCampaign(Guid campaignId, CancellationToken cancellationToken)
    {
        ...
        catch (CampaignDomainException exception)
        {
            ...
            return RedirectToAction("Details", "Campaigns", new { campaignId });
        }
    }
