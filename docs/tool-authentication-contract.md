...
Delegation is explicitly allowlisted on the source Tool registration through `DelegationTargets`. Existing registrations default to no permitted targets. The immediate Dorks & Dice relationship is `character-sheet -> rules-core`; there is no reverse `rules-core -> character-sheet` grant. The Development Tool editor is the authorized configuration surface for these target slugs.
